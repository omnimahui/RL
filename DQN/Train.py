...
from RL_moneyflow.BaseTrain import BaseTrain
from RL_moneyflow.DQN.DeepQNetwork import DQN
...
class Train(BaseTrain):
    def __init__(self,
        ...
        self.policy_net = DQN(env.state_size, len(env.code_to_action)).to(device)
        self.target_net = DQN(env.state_size, len(env.code_to_action)).to(device)
        self.target_net.load_state_dict(self.policy_net.state_dict())
        self.target_net.eval()

        self.optimizer = optim.Adam(self.policy_net.parameters(), lr=0.001)

        self.test_net = DQN(env.state_size, len(env.code_to_action))
